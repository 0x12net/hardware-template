...
- Select board stack
  ...
  4 layers 
  JLC04xx1H-7628 (default) [I Recommend]
  JLC04xx1H-3313 
...
- Choose a license variant or delete the current `LICENSE` file if none applies

  ```
  Three variants of CERN-OHL 2.0

  CERN-OHL-S-2.0 (Strong) — GPL-like for hardware. If you modify and
  distribute a device based on this project, you must disclose the
  source files (schematics/design files) under the same license.
  Maximum copyleft.

  CERN-OHL-W-2.0 (Weak) — LGPL-like. Copyleft applies only to
  modifications of the component itself, not to devices that merely
  use/connect it.

  CERN-OHL-P-2.0 (Permissive) — MIT/BSD-like. Minimal requirements —
  essentially only preserving the attribution notice.
  ```
